...
function liquidFill(_config, _elementId, _value) {
    ...
    var waveHeight = fillCircleRadius * waveHeightScale(getFillLevel(value) * 100);
    ...
    var circleX = d3.scaleLinear().range([
        0, 2 * Math.PI
    ]).domain([0, 1]);
    ...
    var waveAnimateScale = d3.scaleLinear().range([
    ...
    var textRiseScaleY = d3.scaleLinear().range([
        fillCircleMargin + fillCircleRadius * 2,
        (fillCircleMargin + textPixels * 0.7)
    ]).domain([0, 1]);

    // Center the gauge within the parent SVG.
    var elementContainer = element.append('g').attr('transform', 'translate(' + locationX + ',' + locationY + ')');

    // Draw the outer circle.
    var circleArc = d3.arc().startAngle(circleX(0)).endAngle(circleX(1)).outerRadius(circleY(radius)).innerRadius(circleY(radius - circleThickness));
    ...
    var text1 = elementContainer.append('text').text(textRounder(textStartValue) + percentText).attr('class', 'liquidFillGaugeText').attr('text-anchor', 'middle').attr('font-size', textPixels + 'px').style('fill', config.textColor).attr('transform', 'translate(' + radius + ',' + textRiseScaleY(config.textVertPosition) + ')');
    ...
    var waveGroup = elementContainer.append('defs').append('clipPath').attr('id', 'clipWave' + elementId);
    var wave = waveGroup.append('path').datum(data).attr('d', clipArea).attr('T', 0);

    // The inner circle with the clipping wave attached.
    var fillCircleGroup = elementContainer.append('g').attr('clip-path', 'url(#clipWave' + elementId + ')');
    ...
    function updater() {
        this.update = function(value) {
            ...
            var newWavePosition = config.waveAnimate
                ? waveAnimateScale(1)
                : 0;
